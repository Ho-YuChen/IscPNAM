...
import os
import itertools
from data_iter_IscPNAM import DataIterator
import pybedtools
import pdb
import commands

t_flag = commands.getoutput('mkdir tmp_IscPNAM')
pybedtools.set_tempdir('tmp_IscPNAM')
batch_size = 100
...
def load_bigwigs_sc(input_dirs,num_pos,bigwig_lists,input_scATAC_dir):
    ...
    for input_dir in input_dirs:
        ...
        if not os.path.isfile(input_bigwig_info_file):
            ...
            #input_bigwig_files.append('/data2/fly/IscPNAM_data/bigwig_all/'+bigwig_lists[num_pos])
...
def IscPNAMet(out,num_recurrent,num_bws):
