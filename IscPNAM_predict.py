...
Use `IscPNAM_predict.py -h` to see an auto-generated description of advanced options.
"""
import utils_IscPNAM as utils
import numpy as np
...
def main():
    ...
    for num_pos,positive_windows in enumerate(positive_windows_list):
        ...
        try:
            os.stat('%s/IscPNAM_predict_using_H1'%(input_dirs[0]))
        except:
            os.mkdir('%s/IscPNAM_predict_using_H1'%(input_dirs[0]))
            os.mkdir('%s/IscPNAM_predict_using_GM'%(input_dirs[0]))
            os.mkdir('%s/IscPNAM_predict_using_K562'%(input_dirs[0]))
        np.save('%s/IscPNAM_predict_using_H1/%s_data'%(input_dirs[0],str(num_pos)),model_predicts_list[0])
        np.save('%s/IscPNAM_predict_using_GM/%s_data'%(input_dirs[0],str(num_pos)),model_predicts_list[1])
        np.save('%s/IscPNAM_predict_using_K562/%s_data'%(input_dirs[0],str(num_pos)),model_predicts_list[2])
    ...
    t1 = commands.getoutput('rm -f %s/IscPNAM_predict_using_H1/*.npy'%(input_dirs[0]))
    t2 = commands.getoutput('rm -f %s/IscPNAM_predict_using_GM/*.npy'%(input_dirs[0]))
    t3 = commands.getoutput('rm -f %s/IscPNAM_predict_using_K562/*.npy'%(input_dirs[0]))
    '''
